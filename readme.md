...
Koishi 机器人框架的 GitHub Webhook 推送插件，支持将 GitHub 仓库事件推送到 QQ 群聊或私聊，并支持通过 `#<编号>` 快速截图查看 Issue/PR 详情。

## 功能特性

- 🔐 **安全验证** - 支持 HMAC SHA256 签名验证，确保 Webhook 请求来源可信
- 📋 **信任仓库管理** - 管理员可控制哪些仓库的事件可以被处理
- 🔔 **灵活订阅** - 管理员可为群聊/私聊订阅感兴趣的仓库和事件类型
...
- 🖼️ **Issue 截图查询** - 绑定仓库后，发送 `#166` 即可截图查看对应 Issue/PR，支持配置国内代理
...
| `githubProxy` | string | `''` | GitHub 访问代理前缀，如 `https://gh.072118.xyz`（结尾不加斜杠），留空直连 github.com |
| `issueScreenshot.enabled` | boolean | `false` | 启用 Issue 截图功能（需安装 puppeteer 服务） |
| `issueScreenshot.timeout` | number | `15000` | 页面加载超时（毫秒） |
| `issueScreenshot.viewportWidth` | number | `1280` | 截图视口宽度（像素） |
...
plugins:
  github-webhook-pusher:
    ...
    githubProxy: https://gh.072118.xyz
    issueScreenshot:
      enabled: true
      timeout: 15000
      viewportWidth: 1280
...
> 所有带 **（管理员）** 标注的命令需要用户 authority ≥ 3 才能使用。
> 在 Koishi 控制台 → 数据库 → `user` 表中修改对应用户的 `authority` 字段，或使用 `authorize @用户 3` 命令授权。
...
管理员可以为当前群聊/私聊订阅信任列表中的仓库，接收事件通知。
...
### Issue 截图查询（管理员绑定，所有人触发）

管理员为当前群聊/私聊绑定一个仓库后，任何人发送 `#<编号>` 即可查看对应 Issue/PR。

| 命令 | 说明 | 示例 |
|------|------|------|
| `gh.bind <repo>` | 绑定 Issue 查询仓库（管理员） | `gh.bind AUTO-MAS-Project/AUTO-MAS` |
| `gh.unbind` | 解绑 Issue 查询仓库（管理员） | `gh.unbind` |
| `#<编号>` | 截图查看对应 Issue/PR | `#166` |

> 截图功能需要启用 `issueScreenshot.enabled` 并安装 [koishi-plugin-puppeteer](https://www.npmjs.com/package/koishi-plugin-puppeteer)。
> 未安装 puppeteer 时会降级为直接发送链接。
> 服务器在国内时建议配置 `githubProxy` 代理地址。
...
**A:** 请检查：
1. 是否已使用 `gh.bind owner/repo` 绑定了仓库
2. 配置中 `issueScreenshot.enabled` 是否设为 `true`（关闭时只发链接，需要手动发送 `#166`）
3. 服务器在国内时是否配置了 `githubProxy`

### Q: 截图显示不全或加载失败？

**A:** 可以适当增大 `issueScreenshot.timeout` 值（如 `30000`），或检查 `githubProxy` 代理是否可用。
...
- 可选 `puppeteer` 服务（用于 Issue 截图功能，需安装 koishi-plugin-puppeteer）
